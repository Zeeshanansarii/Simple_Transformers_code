...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, dropout_rate, head_size = 8):
        super(MultiHeadAttention, self).__init___()
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, enc_output, self_mask, i_mask, cache):
        ...
        if enc_output is not None:
            ...
            y = self.enc_decf_attention_dropout(y)
            x = x + y
        ...
        for i, dec_layer in enumerate(self.layers):
            ...
            if cache is not None:
                if i not in cache:
                    cache[i] = {}
                layer_cache = cache[i]
                decoder_ouput = dec_layer(decoder_output, enc_output, t_self_mask, i_mask, layer_cache)
            return self.last_norm(decoder_output)



class FastTransformer(nn.Module):
    def __init__(self, i_vocab_size, t_vocab_size, n_layers = 6, hidden_size = 512, filter_size = 2048,
                dropout_rate = 0.1, share_target_embedding = True, has_inputs = True, src_pad_idx = None,
                trg_pad_idx = None):
        ...
        if has_inputs:
            ...
            num_timescale = self.hidden_size // 2
