...
def initialize_weight(x):
    nn.init.xavier_uniform_(x.weights)
    if x.bais is not None:
...
class DecoderLayer(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout_rate):
        ...
        self.fnn_norm = nn.LayerNorm(hidden_size, eps = 1e-6)
...
class Transformer(nn.Module):
    def __init__(self, i_vocab_size, t_vocab_size, n_layers = 6, hidden_size = 512, 
                 ...
                 has_inputs = True, src_pad_idx = None, trg_pad_idx = True):
        ...
        min_timescale = 1.0
        log_timescale_increament = (
            math.log(float(max_timescale) / float(min_timescale)) / max(num_timescale - 1, 1))
        inv_timescale = min_timescale * torch.exp(
            torch.arrange(num_timescales, dtype = torch.float32) * -log_timescale_increment)
