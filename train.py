...
from utils.optimizer import LRSchedular
from utils import utils


def summarize_train(writer, global_step, last_time, model, opt, inputs, targets, optimizer,
                    loss, pred, ans):
    if opt.summary_grad:
    ...
    if inputs is not None:
        ...
        writer.add_scalar('input_stats / inputs_nonpadding_frac', t_nonpad.mean(), global_step)
    ...
    writer.add_scalar('optimizer / learning_rate', optimizer.learning(), global_step)
...
def train(train_data, model, opt, global_step, optimizer, t_vocab_size, label_smoothing, writer):
    model.train()
    last_time = time.time()
    ...
    for batch in train_data:
        ...
        if global_step % 100 == 0:
            summarize_train(writer, global_step, last_time, model, opt, inputs, targets, optimizer,
                            loss, pred, ans)
            last_time = time.time()

        pbar.set_description('[Loss: {:.4f}]'.format(loss.item()))            
        global_step += 1
        pbar.update(targets.size)
...
def validation(validation_data, model, global_step, t_vocab_size, val_writer, opt):
    model.eval()
    total_loss = 0.0
    total_cnt = 0
...
def main():
    ...
    if not os.path.exists(opt.output_dir + '/last/model'):
        os.makedirs(opt.output_dir + '/last/models')
    if not os.path.exists(opt.data_dir):
        os.makers(opt.data_dir)
    ...
    if os.path.exists(opt.output_dir + '/last/models/last_model.pt'):
        ...
        model = model_fn(i_vocab_size, t_vocab_size, n_layers = opt.n_layers,
                         hidden_size=opt.hidden_size, filter_size = opt.filters_size,
                         dropout_rate = opt.dropout, share_target_embedding = opt.share_target_embedding,
                         has_inputs = opt.has_inputs, src_pad_idx = opt.src_pad_idx, trg_pad_idx = opt.trg_pad_idx)
    ...
    num_params = sum(p.numel() for p in model.parameter() if p.requires_grad)
    print("# of parameters: {}".format(num_params))

    optimizer = LRSchedular(
    ...
    for t_step in range(opt.train_step):
        ...
        global_step = train(train_data, model, opt, global_step, optimizer, t_vocab_size, opt.label_smoothing,
                            writer)
        ...
        val_loss = validation(validation_data, model, global_step, t_vocab_size, val_writer, opt)
