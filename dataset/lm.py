...
def read_examples(paths, fields, data_dir, mode, filter_pred, num_shard):
    ...
    for path in paths:
        ...
        with io.open(path, mode='r', encoding='utf-8') as trg_file:
            for trg_line in tdqm(trg_file, ascii=True):
                trq_line = trg_line.strip()
                if trq_line == '':
                    continue

                example = data.Example.fromList([trq_line], fields)
...
class LM1b(data.Dataset):
    ...
    # 1 billion wordlanguage modeling benchmark URL
    ...
    def splits(cls, fields, data_dir, root='.data', **kwargs):
        ...
        train_files = [
            os.path.join(path,
                         ...
                         "news.en-%5d-of-00100" % i) for i in range(1, 100)
        ]

        train_examples, data_paths = \
            read_examples(train_files, fields, data_dir, 'train', filter_pred, 100)
        ...
        val_examples, _= read_examples(val_files, fields, data_dir, 'val', filter_pred, 1)
...
def prepare(max_length, batch_size, device, opt, data_dir):
    ...
    if load_preprocessed:
        ...
        trg_field = data.Field(tokenize = split_tokenizer, batch_first = True, pad_token = pad,
