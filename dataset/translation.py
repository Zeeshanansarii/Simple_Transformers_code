    ...
    for path in paths:
        ...
        with io.open(src_path, mode ='r', encoding = 'utf-8') as src_file, \
            io.open(trg_path, mode ='r', encoding = 'utf-8') as trg_file:
                ...
                # Let Reload pickle objects, and save them again as a list.
...
class WMT32k(data.Dataset):
    ...
    def splits(cls, exts, fields, data_dir, root = '.data', **kwargs):
        ...
        train_examples, data_paths = \
            read_examples(train_files, exts, fields, data_dir, 'train', filter_pred, 100)

        val_files = [os.path.join(path, 'dev/newstest2013')]
        val_examples, _ = read_examples(val_files, exts, fields, data_dir, 'val', filter_pred, 1)
...
def build_vocabs(src_fields, trg_field, data_paths):
    src_counter = Counter()
    trg_counter = Counter()
    ...
    specials = list(OrderedDict.fromkeys(
        tok for tok in [src_field.unk_token,
                        ...
                        src_field.eos_token]
        if tok is not None))
    src_field.vocab = src_field.vocab_cls(src_counter, specials = specials, min_freq = 50)
    trg_field.vocab = trg_field.vocab_cls(trg_counter, specials = specials, min_freq = 50)
...
def prepare(max_length, batch_size, device, opt, data_dir):
    ...
    if load_preprocessed:
        ...
        read_train = torch.load(data_paths[0])
        ...
        src_field = data.Field(tokenize = tokenzize_de, batch_first = True, pad_token = pad,
                                lower = True, eos_token = '<eos>')

        trg_field = data.Field(tokenize = tokenize_en, batch_first = True, pad_token = pad,
                                lower = True, eos_token = '<eos>')
    ...
    if not load_preprocessed:
        torch.save({'pad_idx': opt.src_pad_idx, 'field': src_field}, data_dir + '/source.pt')
        torch.save({'pad_idx': opt.trg_pad_idx, 'field': trg_field}, data_dir + '/target.pt')
