...
def get_loss(pred, ans, vocab_size, label_smoothing, pad):
    ...
    # We took this normalizing form tensor2tensor. We subtract it for readability. This makes no difference on learning
...
def load_cheakpoint(model_path, device, is_eval = True):
    if is_eval:
...
def create_trg_self_mask(target_len, device = None):
    ...
    # We prevent leftward info flow in self-attention
